  ...
  return {
    ...
    Stepper: {
      ...
      Navigation: ({ children, "aria-label": ariaLabel = "Stepper Navigation", ...props }) => {
        ...
        return (
          <nav data-component="stepper-navigation" aria-label={ariaLabel} {...props}>
            <ol data-component="stepper-navigation-list" className={classForNavigationList({ variant: variant })}>
              {children}
            </ol>
          </nav>
        );
      ...
      Panel: ({ children, asChild, ...props }) => {
        ...
        return (
          <Comp data-component="stepper-step-panel" ref={(node: HTMLDivElement | null) => scrollIntoStepperPanel(node, tracking)} {...props}>
            {children}
          </Comp>
        );
      },
      Controls: ({ children, className, asChild, ...props }) => {
        const Comp = asChild ? Slot : "div";
        return (
          <Comp data-component="stepper-controls" className={cn("flex justify-end gap-4", className)} {...props}>
            {children}
          </Comp>
        );
      },
...
const Title = ({ children, className, asChild, ...props }: React.ComponentProps<"h4"> & { asChild?: boolean }) => {
  ...
  return (
    <Comp data-component="stepper-step-title" className={cn("text-base font-medium", className)} {...props}>
      {children}
    </Comp>
  );
};
